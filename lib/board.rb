...
class Board
  ...
  attr_reader :cells
  def initialize(user_input_x = 4, user_input_y = 4)
    ...
    number = 1
    @cells = {}
    (user_input_x * user_input_y).times do

      combined = alpha_character.chr.to_s + number.to_s

      @cells[combined] = Cell.new(combined)
